...
namespace NTP_P1
{
    public partial class RegisterPage : Form
    {
        ...
        public static String Verify_Code, mail_address, password, password_check, username, root, Fname,Sname;
        public static string default_user;
        public static bool mail_Existence, username_Existence, isTry;
        ...
        private async void label2_MouseHover(object sender, EventArgs e)
        {
            label2.Visible = true;
            await Task.Delay(3000);
        ...
        {
            checkBox2.BackColor = DarkenColor(checkBox2.BackColor, 0.2f);
        }
        ...
        private async void pictureBox6_MouseHover(object sender, EventArgs e)
        {
            label5.Visible = true;
            await Task.Delay(3000);
        ...
        private async void pictureBox8_MouseHover(object sender, EventArgs e)
        {
            label6.Visible = true;
            await Task.Delay(3000);
        ...
        private void btnVerifyCodeSend_Click(object sender, EventArgs e)
        {
            ...
            root = "0";
            Program.verifyType = "R";
            username = textBox2.Text;
            Fname = textBox3.Text;
            Fname = Program.EncryptIt(Fname);
            Sname = textBox4.Text;
            Sname = Program.EncryptIt(Sname);
            username = Program.EncryptIt(username);
            username_Existence = db.CheckUsernameExists(username);
            //ShowForATime(updateBtn);
            Program.CursorChange();
            //AnimatedRotate(updateBtn);

            if (IsValidEmail(textBox1.Text) && textBox2.Text.Length > 1 && textBox3.Text.Length > 0 && textBox4.Text.Length > 0)
            {
                if (IsValidPassword(sifre.Text))
                {
                    if (username_Existence)
                    {
                        MessageBox.Show(Program.Output[20], Program.Output[19], MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
                    else
                    {
                        ...
                        mail_address = Program.EncryptIt(mail_address);
                        mail_Existence = db.CheckEmailExists(mail_address);
                        if (mail_Existence)
                        {
                            MessageBox.Show(Program.Output[21], Program.Output[19], MessageBoxButtons.OK, MessageBoxIcon.Error);
                        }
                        else
                        {
                            password = sifre.Text;
                            password_check = sifreCheck.Text;
                            ...
                            if (sifre.Text == sifreCheck.Text)
                            {
                                if (sifre.TextLength >= 4)
                                {
                                    try
                                    {
                      ...
                      <tbody><tr>
                        <td style=""vertical-align:top;text-align:center"">
                          <div style=""display:inline-block;padding-top:20px"">
                            <img src='cid:{linkedResource.ContentId}' alt='Embedded Image' width=""auto"" height=""auto"" data-bit=""iit"">
                          </div>
                        </td>
                      </tr>
                      <tr>
...
Hey {textBox3.Text} {textBox4.Text},
</div>
...
<table cellspacing=""0"" cellpadding=""0"" style=""padding-top:16px"">
        <tbody><tr>
            <td style=""padding-left:14px"">
               <div style=""font-weight:400;font-size:12px;color:#fff"">{Program.Output[23]}</div>
               <div style=""font-weight:400;font-size:12px;color:#fff""><h2>[{Verify_Code}]</h2></div>
                <div style=""font-weight:400;font-size:12px;color:#fff"">{Program.Output[24]}</div>
            ...
            <td valign=""top"">
                <div style=""width:32px;height:32px;background-color:rgba(83,252,24,0.15);border-radius:4px;text-align:center"">
                    <table style=""width:100%;height:100%"">
						...
						<img src='cid:{linkedResource2.ContentId}' alt='Embedded Image' style=""width:32px;height:32px;"" data-bit=""iit"">
						</td></tr>
                    </tbody></table>
                </div>
            </td>
    ...
    <tr>
        ...
        <td align=""right"">
            <table cellpadding=""0"" cellspacing=""0"" role=""presentation"">
                ...
                {
                    MessageBox.Show($"{Program.Output[135]}\n" +
                        $"[^\\s'\"\\\\%&/#=?!*]");
                }
            }
            else
            {
                ...
                {
                    MessageBox.Show(Program.Output[2]);
                }
